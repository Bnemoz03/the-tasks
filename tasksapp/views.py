...
def sign_up_user(request):
    ...
    if is_get(request):
        ...
        eligible_password: bool = len(password1) >= 6

        if passwords_match and full_fields and eligible_password:
            try:
...
def completed_tasks(request):
    ...
    tasks = tasks_filtered.order_by('-completed_time')
    return render(
        request, 
